...
pipeline {
    ...
    environment {
        SONARQUBE_PROJECT_KEY = "vtb-testing"
    ...
    stages {
        ...
        stage('Publish') {
            steps {
                sh 'dotnet publish vtb.Testing.Rest/vtb.Testing.Rest.csproj --no-build --no-restore -o ./out -c Release'
                archiveArtifacts artifacts: 'out/*', fingerprint: true

                sh 'dotnet pack vtb.Testing.Rest/vtb.Testing.Rest.csproj --no-build --no-restore -p:PackageVersion=${VERSION} -o . -c Release'
                archiveArtifacts artifacts: '*.nupkg', fingerprint: true

                sh 'dotnet nuget push "vtb.Testing.Rest.${VERSION}.nupkg" -s BaGet -k ${BAGET_API_KEY}'
            }
        }
    }
